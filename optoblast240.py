...
origin_dir = r"G:\Shared drives\Lybrand MEA Drive\Opto blast\hd5 files\opto blast 8w\OptoPharm stim"
or_dir = r"G:\Shared drives\Lybrand MEA Drive\Optoblast Data\OptoPharm stim\Stim CSV Files"
dest_dir = r"G:\Shared drives\Lybrand MEA Drive\Optoblast Data\OptoPharm stim 120"
nf = 120 #number of subsegments desired
...
for file in os.listdir(or_dir):
    well = str(file[:-4])
    if well not in os.listdir(dest_dir):
        print(file)
        ...
        for i in range(nf):
            #split into 48 files
            df = control.iloc[int(i * (len(control)/nf)): int((i+1) * len(control)/nf), :]
                    ...
                    for chan2 in connections[channel]:
                        if chan2 != chan:
                            if chan in connections[chan2]:
                                sum_connections += 1
                try:
                    cluster_coeff[channel] = sum_connections / 2 / nCr(len(connections[channel]),
                                                                       2)  # calculates clustering coefficients for each channel/node
                except:
...
for well in os.listdir(dest_dir):
    if "Stim CSV" not in str(well):
            ...
            try:
                ...
                for hub in hubs[1:]:
                    for network in networks:
                        for node in network:
                            if hub in hub_nodes[node]:
                                network.append(hub)
                            else:
                                if not any(hub in netwrk for netwrk in networks):
                                        networks.append([hub])
                for network in networks:
        ...
        pd.DataFrame({'Phases':phases, 'Independent Connections':ind_connections}).to_csv(os.path.join(synch_cc_data, str(well) + '_Independent_Connections.csv'))
